...
<body>
    ...
    <script>
       ...
       xdd.onclick = function(){
            ...
            //开启定时器
            window.setTimeout(function(){   ////window可省略
                console.log(this)
                //变粉色
...
<script>
    ...
    fun3(1,2,3)
    fun4(1,2,3,4)
    // rest参数只在一个参数列表中的最后一个,这也就无形之中要求一个参数列表中只能有一个rest参数
...
- JSON.parse                   JSON串转对象
- JSON.stringify(person) 对象转JSON串
...
+ 如果需要修改全局依赖的安装路径，可以按照以下步骤操作：
    ...
    1. 创建一个新的全局依赖存储目录，例如 `E:\JavaInact\GlobalNodeModules`。

    2. 打开命令行终端，执行以下命令来配置新的全局依赖存储路径：

        ``` shell
        npm config set prefix "E:\JavaInact\GlobalNodeModules"
        ```
...
+ npm install 包名 或者 npm install 包名@版本号
    ...
    + 安装后会更改package.json内容，记录下载的依赖
+ npm install -g 包名
    + 安装全局依赖包(安装到"E:\JavaInact\GlobalNodeModules"则可以在任何项目中使用它，而无需在每个项目中独立安装该包。
+ npm install
...
![](E:\JavaInact\JavaWeb\笔记\images\父子传参1.png)

![](E:\JavaInact\JavaWeb\笔记\images\父子传参2.png)

![](E:\JavaInact\JavaWeb\笔记\images\父子传参3.png)
...
```html
![路由1](E:\JavaInact\JavaWeb\笔记\images\路由1.png)<script setup>
</script>
...
![](E:\JavaInact\JavaWeb\笔记\images\路由1.png)
...
<template>
    <div>
      ...
      <hr/>
        ...
        <input type="text" v-model="routePath">
      <hr/>
      <!-- 路由连接对应视图的展示位置 -->
      <hr>
      默认展示位置:<router-view></router-view>
...
```
![](E:\JavaInact\JavaWeb\笔记\images\路由2.png)

![](E:\JavaInact\JavaWeb\笔记\images\路由3.png)
...
- promise
  ...
  then里面  第一个函数是成功时调用   第二个函数是失败是调用   中间用逗号隔开
