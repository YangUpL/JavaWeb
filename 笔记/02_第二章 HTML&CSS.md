...
+ 设置左侧树缩进: 齿轮>设置>搜索 "workbench>tree"
...
+ form标签,表单标签,其内部用于定义可以让用户输入信息的表单项标签
    ...
    + method, form标签的属性之一,用于定义信息的提交方式
        + get    get方式, 数据会缀到url后,以?作为参数开始的标识,多个参数用&隔开
          + input标签必须定义name属性,才能显示key=value
...
+ 说明
  + readonly  只读提交
    ...
    + ```html
      个性签名：<input type="text" name="signal" value="111" readonly/><br/>
      ```
  + disabled  不提交
  + value默认值
...
- 说明
  - 块(div)    css样式一般生效(不在同一行)
  - 层(行内元素)(span)   css 样式不一定生效(作用: 突出显示)
...
+ 说明
    + absolute ,通过 top left right bottom 指定元素在页面上的固定位置(相对浏览器)
