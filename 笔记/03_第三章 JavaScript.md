...
+ ```js
  onkeydown="testDown1(),testDown2()" //键盘按下
  onfocus="testFocus()"    //获得焦点
  onblur="testBlur()"    //失去焦点
  onchange="testChange(this)"  //内容改变
  onmouseover="testMouseOver()"    //鼠标悬停
  onmouseleave="testMouseLeave()"  //鼠标离开
  onmousemove="testMouseMove()"    //鼠标移动
  ```
...
<body>
    <!-- 当testSubmit() return true/false时绑定事件需要继续return给浏览器-->
    <form action="01js的引入方式.html" method="get" onsubmit=" /*return*/ testSubmit()" onreset="testReset()">
        <input type="text" 
...
+ 说明
    ...
    + 一个事件可以同时绑定多个函数
    + 一个元素可以同时绑定多个事件
    + 方法中可以传入 this对象,代表当前元素(this.value取值)

> 通过DOM编程绑定

- 必须先执行<input id="in1" type="text" />
  - 将相关操作放在windows.onload = function()里,当页面全部扫描完毕后,自动触发onload事件
...
<head>
    ...
    <script>
        ...
        window.onload=function(){
            var in1 =document.getElementById("in1");
            console.log(in1);//获得的是整个标签

            // 通过DOM编程绑定事件
            in1.onchange = function(){
                console.log("内容改变")
                console.log(event.target);//这里也是整个标签
                console.log(event.target.value);//获取标签属性
                console.log(event.target.id);//获取标签属性
            }
...
>  三种弹窗方式/定时: window对象的方法
>
>  history/location/console/sessionStorage/localStorage : window属性的方法
>
>  window可以省略不写
...
<body>
    ...
    <input type="button" value="两秒后向控制台打印hello" 
                onclick="testTimeout()"/> <br>

    <hr/><hr/><br/>

    <a href="http://www.atguigu.com">尚硅谷</a>
    <button onclick="testBack()">上一页</button>
    <button onclick="testTowards()">下一页</button>


    <hr/><hr/><br/>
    <button onclick="testHref()">尚硅谷</button>
...
![](E:\JavaInact\JavaWeb\笔记\images\dom1.jpg)
...
<head>
   ...
   function changeAttribute(){
        ...
        // 语法 元素.属性名=""
        // 获得属性值
        console.log(in1)//在修改之前输出  标签也已经变了 
...
<body>
    ...
    <button id="hhh" style="background-color: aqua;">测试</button>
